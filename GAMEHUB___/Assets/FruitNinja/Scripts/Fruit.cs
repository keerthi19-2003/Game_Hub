...
namespace FruitNinja.Scripts {
    public class Fruit : MonoBehaviour {
        ...
        private void Slice(Vector3 direction, Vector3 position, float force) {
            ...
            _gameManager.AddTime(3f); // Award +3 seconds for slicing
            UpdateFruitState();
            PlayJuiceEffect();
        ...
        private void ApplyForceToSlices(Vector3 direction, Vector3 position, float force) {
            ...
            foreach (var slice in slices) {
                slice.linearVelocity = _fruitRb.linearVelocity;
        ...
        // New method to handle unsliced fruit
        private void OnBecomingInvisible() {
            if (whole.activeSelf) {
                _gameManager.MissedFruit();
                Destroy(gameObject); // Remove the fruit object
            }
        }
        
        // Alternative approach using LateUpdate
        private void LateUpdate() {
            if (whole.activeSelf && !IsInCameraView()) {
                _gameManager.MissedFruit();
                Destroy(gameObject); // Remove the fruit object
            }
        }
